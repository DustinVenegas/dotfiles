...
```ps1
# In an elevated (Administrator) PowerShell Console. Use `csudo` on ConEmu!
...
Neovim and your global python library. `pipenv` also manages `pipfile`
...
```ps1
# From any directory. pipenv can create/manage python venv dependencies
...
let g:python3_host_prog='C:\Users\dustin\.virtualenvs\3.7-LFOTLC8X\Scripts\python.exe'
let g:python_host_prog='C:\Users\dustin\.virtualenvs\2.7-8oiZRLVE\Scripts\python.exe'
...
```viml
" Add our dotfiles checkout as a neovim runtimepath for running scripts.
set runtimepath^=~/dotfiles/nvim/
